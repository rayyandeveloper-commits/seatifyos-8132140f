...
import { Crown, Clock, TrendingUp, PieChart as PieIcon, IndianRupee } from "lucide-react";
import { Shell } from "@/components/layout/Shell";
import { StatCard } from "@/components/ui-blocks/StatCard";
import { SkeletonStatCard } from "@/components/ui-blocks/SkeletonCard";
import { useCabinHistory, useCabins, useStudents, usePayments } from "@/lib/queries";
...
function Analytics() {
  ...
  const { perCabin, longest, monthly, utilization } = useMemo(() => {
    ...
    for (const h of history) {
      ...
      prev.sessions += 1; prev.days += days;
  ...
  const revenueData = useMemo(() => {
    if (payError || payments.length === 0) return [];
    ...
    return [...byMonth.entries()].sort().map(([k, v]) => ({ month: k.slice(2), revenue: v }));
  }, [payments, payError]);
  ...
  return (
    <Shell title="Analytics" subtitle="Occupancy, revenue & history insights">
      {/* KPI stat cards */}
      ...
      {/* Revenue section — only when payments table exists */}
      {!payError && revenueData.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.05 }}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 rounded-xl glass px-3 py-1.5">
              <IndianRupee className="h-3.5 w-3.5 text-[oklch(0.78_0.18_155)]" />
              <span className="font-display text-sm font-semibold">{fmt(totalRevenue)} total collected</span>
            </div>
          </div>
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={revenueData}>
                ...
                <YAxis stroke="oklch(0.70 0.03 270)" fontSize={11} tickFormatter={(v) => `₹${(v / 1000).toFixed(0)}k`} />
                <Tooltip contentStyle={TS} formatter={(v) => [fmt(Number(v)), "Revenue"]} />
                <Line type="monotone" dataKey="revenue" stroke="oklch(0.72 0.18 155)" strokeWidth={2.5}
                  dot={{ fill: "oklch(0.72 0.18 155)", strokeWidth: 0, r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
              ...
              {topCabins.length === 0 ? (
                <div className="flex h-72 items-center justify-center text-sm text-muted-foreground">
                  No data yet — cabin assignments appear here.
                </div>
              ) : (
